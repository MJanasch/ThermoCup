...
addpath('Model/')
addpath('Support/')
modelFile = '/Model/Cupriavidus_Core_XIV_PHB.xlsx';
CupriaCore = ThermoCup_ReadExcel_II(modelFile);

%% Prepare structure for EFM enumeration via EFMtool

% Need to move to EFMtool directory or add path
% cd ('.../efmtool/')
stru.stoich             = CupriaCore.Variant_Frc.rawS;                  % Change to CupriaCore.Variant_Frc.rawS or CupriaCore.Variant_For.rawS respecetively
stru.reversibilities    = CupriaCore.Variant_Frc.reactions.reversible;  % Change to CupriaCore.Variant_Frc.reactions.reversible or CupriaCore.Variant_For.reactions.reversible respecetively
stru.metaboliteNames    = CupriaCore.metabolites.mets;                  % Can stay the same for both variants
stru.reactionNames      = CupriaCore.Variant_Frc.reactions.rxns;        % Change to CupriaCore.Variant_Frc.reactions.rxns or CupriaCore.Variant_For.reactions.rxns, respectively

% Enumerate EFMs via EFMtool
% tic
% mnet_Frc = CalculateFluxModes(stru);           % Change to mnet_Frc or mnet_For, repsectively
% toc
...
for i = 1:length(mnet.efms)
    ...
    % if (((mnet.efms(4,i) ~= 0) && mnet.efms(9,i) ~= 0) || (mnet.efms(4,i) ~= 0 && mnet.efms(9,i) == 0) || (mnet.efms(4,i) == 0 && mnet.efms(9,i) ~= 0)) && mnet.efms(Xfpk1,i) == 0 && mnet.efms(Xfpk2,i) == 0 && mnet.efms(AclAB,i) == 0
        ...
        if MDF(r) > 0    % If the EFM is thermodynamically feasible
            ...
            yield_PHB(r_yes)        = (mnet.efms(9,i)*MolWeight_PHB)/(mnet.efms(1,i)*MolWeight);
...
for i = 1:length(mnet.efms)
    ...
    %if (((mnet.efms(4,i) ~= 0) && mnet.efms(9,i) ~= 0) || (mnet.efms(4,i) ~= 0 && mnet.efms(9,i) == 0) || (mnet.efms(4,i) == 0 && mnet.efms(9,i) ~= 0)) && mnet.efms(Xfpk1,i) == 0 && mnet.efms(Xfpk2,i) == 0 && mnet.efms(AclAB,i) ~= 0
        ...
        if MDF_aclAB(w) > 0     % If the EFM is thermodynamically feasible
            ...
            yield_PHB_aclAB(w_yes)          = (mnet.efms(9,i)*MolWeight_PHB)/(mnet.efms(1,i)*MolWeight);
...
disp(['Total number of feasible aclAB-EFMs found: ',num2str(w_yes)])
toc
    ...
    %if (((mnet.efms(4,i) ~= 0) && mnet.efms(9,i) ~= 0) || (mnet.efms(4,i) ~= 0 && mnet.efms(9,i) == 0) || (mnet.efms(4,i) == 0 && mnet.efms(9,i) ~= 0)) && (((mnet.efms(Xfpk1,i) ~= 0) && mnet.efms(Xfpk2,i) ~= 0) || (mnet.efms(Xfpk1,i) ~= 0 && mnet.efms(Xfpk2,i) == 0) || (mnet.efms(Xfpk1,i) == 0 && mnet.efms(Xfpk2,i) ~= 0)) && mnet.efms(AclAB,i) == 0
       ...
       [MDF_xfpk(v)] = ThermoCup_MDF(Model_Variant,CupriaCore.metabolites,CupriaCore.cofaratios,mnet.efms(:,i));
        if MDF_xfpk(v) > 0     % If the EFM is thermodynamically feasible
            ...
            yield_PHB_xfpk(v_yes)          = (mnet.efms(9,i)*MolWeight_PHB)/(mnet.efms(1,i)*MolWeight);
...
disp(['Total number of feasible Xfpk-EFMs found: ',num2str(v_yes)])
toc